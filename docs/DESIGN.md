...
> Design notes for **weekly-shopper-order-digest** — a runnable typescript example that Queue typed order updates and compile the latest checkout, fulfillment, and receipt state on a weekly cron.
...
- **Feature code** — the actual task: weekly-order-digest.
